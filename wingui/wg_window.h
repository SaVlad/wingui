...
namespace wg::controls {
	...
	enum WindowStyleEx {
		...
		WSAcceptFiles = WS_EX_ACCEPTFILES,
		WSSunkenEdge = WS_EX_CLIENTEDGE,
		WSHasHelp = WS_EX_CONTEXTHELP,
		WSIsModal = WS_EX_DLGMODALFRAME,
		WSNoActivate = WS_EX_NOACTIVATE,
		WSCommandDialog = WS_EX_PALETTEWINDOW,
		WSToolWindow = WS_EX_TOOLWINDOW,
		WSTopMost = WS_EX_TOPMOST,
		WSRaisedEdge = WS_EX_WINDOWEDGE
	};
	MAKEFLAGS(WindowStyleEx);
	enum WindowClassStyle {
		WCSNone = 0,
		WSReceiveDoubleClick = CS_DBLCLKS,
		WSDropShadow = CS_DROPSHADOW,
		WSNoClose = CS_NOCLOSE,
		WSMoveSizeRedraw = CS_VREDRAW | CS_HREDRAW
	};
